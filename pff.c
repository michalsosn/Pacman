...
static FRESULT dir_rewind(DIR *dj) {
	...
	dj->sect = clst ? clust2sect(clst) : fileSystem->dirbase;	/* Current sector */
...
FRESULT pf_read(void* buff,	tU16 btr, tU16* br) {
	...
	*br = 0;
	...
	while (btr)	{									/* Repeat until all data transferred */
		...
		dr = disk_readp(!buff ? 0 : rbuff, fileSystem->dsect, (tU16)(fileSystem->fptr % 512), rcnt);
		...
		printf("Po goto");
		fileSystem->fptr += rcnt; rbuff += rcnt;
